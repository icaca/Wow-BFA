...
function AuctionDB.RunQuickScan()
    if private.isScanning then
        Log.PrintUser(L["ERROR: 上一次的完整扫描尚未完成."])
        return
    end
    if not private.ahOpen then
        Log.PrintUser(
            L["ERROR: The auction house must be open in order to do a scan."])
        return
    end
    counts = 1
    isScanning = true
    TSM.db.factionrealm.internalData.auctionDBScanTime = time()
    TSM.db.factionrealm.internalData.csvAuctionDBScan = ""
    Event.Register("AUCTION_HOUSE_BROWSE_RESULTS_ADDED", private.ScanNext)
    Log.PrintUser("正在扫描第" .. counts .. "页")
    list = C_AuctionHouse.GetBrowseResults()
    private.DealData(list)
    C_AuctionHouse.RequestMoreBrowseResults()
end
function private.ScanNext(event, list)
    if isScanning then
        isScanning = false
        if event == "AUCTION_HOUSE_BROWSE_RESULTS_UPDATED" then
            list = C_AuctionHouse.GetBrowseResults()
        end
        if #list > 0 then private.DealData(list) end
        if not C_AuctionHouse.HasFullBrowseResults() then
            ...
            Log.PrintUser("扫描完毕!")
            counts = 1
            Event.Unregister("AUCTION_HOUSE_BROWSE_RESULTS_ADDED",
                             private.ScanNext)
        end
    end
end

function private.DealData(list)
    for _, data in ipairs(list) do
        local baseItemString = ItemString.GetBaseFromItemKey(data.itemKey)
        local minPrice = data.minPrice
        local totalQuantity = data.totalQuantity
        private.scanRealmData[baseItemString] =
            private.scanRealmData[baseItemString] or {
                minBuyout = minPrice,
                marketValue = minPrice,
                numAuctions = totalQuantity,
